<x-layouts.toolbox title="Edit Layup" :breadcrumbs="[
    ...
    <div class="max-w-md">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Edit Layup</h1>
            <p class="text-sm text-gray-400 mt-0.5">Editing <span class="font-medium text-gray-600">{{ $layup->name }}</span>.</p>
        </div>

        <div class="bg-white rounded-2xl border border-neutral-200 shadow-sm p-6">
            <form method="POST" action="{{ route('suppliers.layups.update', [$supplier, $layup]) }}" class="space-y-5">
                @csrf @method('PATCH')
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              ...
                              style="--tw-ring-color:#2d6a4f4d;">{{ old('description', $layup->description) }}</textarea>
